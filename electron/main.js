...
function registerIpc() {
  ...
  ipcMain.handle('pairing:confirmIncoming', (_e, { sessionId, code } = {}) => { const result = pairingServer?.confirm?.(sessionId, code) || { ok: false, reasonCode: 'PAIRING_SERVICE_UNAVAILABLE' }; if (result.ok) credentialStore.set(result.remoteDeviceId, JSON.stringify({ authorization: result.authorization, fingerprint: result.remoteFingerprint })); return result; });
