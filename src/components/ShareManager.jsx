    ...
    <div className="form">
      ...
      {sys.platform === 'win32' && (
        <label style={{ flexDirection: 'row', alignItems: 'center', gap: 8, fontSize: 12.5, color: 'var(--text)' }}>
          ...
          <span>统一账号模式（使用当前 Windows 登录账号：{sys.username || '?'}，所有共享共用）</span>
        </label>
      )}
      {unified ? (
        <label>
          账号（统一账号）
          <input value={account} readOnly placeholder={sys.username || ''} />
        </label>
      ) : (
        <label>
          账号
      ...
      <p className="hint">
        {unified
          ? '统一账号模式：所有共享共用当前 Windows 登录账号，不新建账号、删除时不删登录账号。请填写该账号的登录密码（Mac 端挂载用）。'
...
function SshPanel() {
  ...
  const [portInput, setPortInput] = useState('2222');
  ...
  const refresh = useCallback(async () => {
    try {
      const s = await window.api.ssh.status();
      setStatus(s);
      if (s && s.port) setPortInput(String(s.port));
    } catch (e) {
  ...
  const loadKeys = useCallback(async () => {
    try {
      setKeys(await window.api.ssh.listKeys());
    } catch (e) {
      show(String(e.message || e), false);
    }
  }, []);
  ...
  const enable = async () => {
    setBusy(true);
    try {
      await window.api.ssh.enable();
      show('OpenSSH Server 已启用');
      refresh();
    } catch (e) {
      show(String(e.message || e), false);
    } finally {
  ...
  const control = async (action) => {
    setBusy(true);
    try {
      const s = await window.api.ssh.control(action);
      setStatus(s);
      show(action === 'start' ? 'SSH 服务已启动' : 'SSH 服务已停止');
    } catch (e) {
      show(String(e.message || e), false);
    } finally {
      setBusy(false);
  ...
  const applyPort = async () => {
    setBusy(true);
    try {
      const s = await window.api.ssh.setPort(portInput.trim());
      setStatus(s);
      if (s && s.port) setPortInput(String(s.port));
      show(`端口已改为 ${s.port}，服务已重启`);
    } catch (e) {
      show(String(e.message || e), false);
    } finally {
      setBusy(false);
  ...
  const addKey = async () => {
    if (!pubkey.trim()) return show('请先粘贴 Mac 的公钥', false);
    setBusy(true);
    try {
      await window.api.ssh.addKey(pubkey);
      show('公钥已授权，Mac 可免密同步清单');
      setPubkey('');
      loadKeys();
    } catch (e) {
      show(String(e.message || e), false);
  ...
  const removeKey = async (k) => {
    if (!confirm(`撤销该公钥授权？\n${k.comment || k.type} ${k.key ? k.key.slice(0, 20) + '…' : ''}`)) return;
    setBusy(true);
    try {
      await window.api.ssh.removeKey(k.line);
      show('已撤销授权');
      loadKeys();
    } catch (e) {
      show(String(e.message || e), false);
  ...
  return (
    <div className="ssh-panel">
      <div className="ssh-head">
        <div>
          <h3>SSH 免密通道</h3>
          <p className="sub">启用后 Mac 可一键免密同步共享清单，无需手动填账号</p>
        </div>
        <span className={`badge ${status?.running ? 'ok' : 'warn'}`}>
          {!status ? '检测中…' : status.installed ? (status.running ? `SSH 运行中 :${status.port || 2222}` : '已安装未运行') : '未安装'}
        </span>
      </div>
      <div className="ssh-body">
        {/* 启停 + 端口配置 */}
        <div className="ssh-ctl-row">
          {!status?.installed ? (
            <button className="btn small" onClick={enable} disabled={busy}>
              {busy ? '启用中…' : '⚙ 启用 OpenSSH Server'}
            </button>
          ) : status.running ? (
            <button className="btn small danger" onClick={() => control('stop')} disabled={busy}>
              停止服务
            </button>
          ) : (
            <button className="btn small" onClick={() => control('start')} disabled={busy}>
              启动服务
            </button>
          )}
          <span className="ssh-port-group">
            <input
              ...
              style={{ width: 80 }}
            />
            <button className="btn small" onClick={applyPort} disabled={busy || !portInput.trim()}>
              应用端口并重启
            </button>
          </span>
        </div>
        <div className="ssh-key-row">
          <textarea
            ...
            rows={3}
            placeholder="粘贴 Mac 端生成的公钥（ssh-ed25519 AAAA…），粘贴后点「授权」"
            value={pubkey}
            onChange={(e) => setPubkey(e.target.value)}
          />
          <button className="btn small primary" onClick={addKey} disabled={busy || !pubkey.trim()}>
            授权
          </button>
        </div>
        {/* 已授权公钥管理 */}
        <div className="ssh-keys-block">
          <div className="ssh-keys-head">
            <span className="k">已授权公钥（{keys.length}）</span>
            <button className="btn small" onClick={loadKeys} disabled={busy}>
              刷新
            </button>
          </div>
          {keys.length === 0 ? (
            <p className="hint">暂无已授权公钥。粘贴 Mac 公钥后点「授权」。</p>
          ) : (
            <div className="ssh-keys-list">
              {keys.map((k, i) => (
                <div className="ssh-key-item" key={i}>
                  <span className="ssh-key-type">{k.type}</span>
                  <span className="ssh-key-comment">{k.comment || k.key.slice(0, 24) + '…'}</span>
                  <button className="btn small danger" onClick={() => removeKey(k)} disabled={busy}>
                    撤销
                  </button>
                </div>
              ))}
        ...
        <p className="hint">授权步骤：Mac 端「SSH 同步」页复制公钥 → 粘贴到这里授权 → 之后 Mac 点「拉取清单」即免密同步</p>
      </div>
