...
test('配对服务只向本机通知验证码，支持允许、拒绝和状态查询', async () => {
  ...
  try {
    ...
    const accepted = await (await fetch(`http://127.0.0.1:${service.port}/api/pair/status/${requested.sessionId}`)).json(); assert.equal(accepted.state, 'accepted');
